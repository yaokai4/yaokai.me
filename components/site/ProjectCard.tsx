...
export function ProjectCard({ project, variant = "default" }: { project: Project; variant?: "default" | "featured" }) {
  ...
  return (
    <Link href={`/projects/${project.slug}`} className={cn("premium-glass-card group grid overflow-hidden rounded-md p-0", isFeatured && "lg:grid-cols-[0.95fr_1.05fr]")}>
      <div className={cn("relative overflow-hidden bg-cyan-50", isFeatured ? "min-h-[260px] lg:min-h-full" : "aspect-[16/9]")}>
        {project.coverImage ? (
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(255,255,255,0.34),transparent_36%),linear-gradient(180deg,rgba(255,255,255,0.08),rgba(15,23,42,0.16))]" />
        {project.featured ? <span className="absolute left-4 top-4 rounded-md border border-white/70 bg-white/78 px-3 py-1.5 text-xs font-black text-cyan-800 shadow-sm backdrop-blur">Featured Case</span> : null}
      </div>
      <div className={cn("p-5", isFeatured && "flex flex-col justify-center p-6 md:p-8")}>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {project.techStack.slice(0, 4).map((tech) => (
            <span key={tech} className="rounded-md border border-cyan-100 bg-cyan-50/70 px-2 py-1 text-xs font-bold text-cyan-800">{tech}</span>
          ))}
        </div>
