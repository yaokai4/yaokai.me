...
import { assertProfileDownloadable, canUseOwnAccessProfile, createProfileDownloadToken, getMyAccessProfile } from "@/lib/access-profiles";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!canUseOwnAccessProfile(actor)) return fail("当前角色不能生成下载链接。", "FORBIDDEN", 403);
    const profile = await getMyAccessProfile(actor, false);
    if (!profile) return fail("尚未生成访问配置。", "NOT_FOUND", 404);
    if (profile.status !== "active") return fail("只有 active 配置可以下载。", "FORBIDDEN", 403);
    await assertProfileDownloadable(String(profile.id));

    const result = await createProfileDownloadToken({
    ...
    if (error instanceof Error && error.message === "Unauthorized") return fail("未登录，无法生成下载链接。", "UNAUTHORIZED", 401);
