...
import { canUseOwnAccessProfile, getMyAccessProfile, rotateAccessProfile, sanitizeAccessProfile } from "@/lib/access-profiles";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!canUseOwnAccessProfile(actor)) return fail("当前角色不能重新生成配置。", "FORBIDDEN", 403);
    ...
    const profile = await rotateAccessProfile(String(current.id));
    await writeVpsAuditLog({
      request,
      actor,
      action: "access_profile_rotated",
    ...
    if (error instanceof Error && error.message === "Unauthorized") return fail("未登录，无法重新生成配置。", "UNAUTHORIZED", 401);
