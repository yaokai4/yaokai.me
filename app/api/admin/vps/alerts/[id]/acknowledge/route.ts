...
export async function POST(request: NextRequest, { params }: Context) {
  try {
    ...
    const before = await prisma.vpsAlert.findUnique({ where: { id } });
    const alert = await prisma.vpsAlert.update({ where: { id }, data: { status: "acknowledged", acknowledgedBy: actor.email, acknowledgedAt: new Date() } });
    await writeVpsAuditLog({ request, actor, action: "alert_acknowledged", targetType: "vps_alert", targetId: id, before, after: alert });
    return ok(alert);
  } catch (error) {
