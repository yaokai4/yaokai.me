...
export async function POST(request: NextRequest, { params }: Context) {
  try {
    ...
    const alert = await prisma.vpsAlert.update({ where: { id }, data: { status: "ignored" } });
    await writeVpsAuditLog({ request, actor, action: "alert_ignored", targetType: "vps_alert", targetId: id, before, after: alert });
    return ok(alert);
  } catch (error) {
