...
export async function POST(request: NextRequest, { params }: Context) {
  try {
    ...
    const alert = await prisma.vpsAlert.update({ where: { id }, data: { status: "resolved", resolvedAt: new Date() } });
    await writeVpsAuditLog({ request, actor, action: "alert_resolved", targetType: "vps_alert", targetId: id, before, after: alert });
    return ok(alert);
  } catch (error) {
