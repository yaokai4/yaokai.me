...
最重要的变化不是技术，而是团队不再把重复手工劳动视为理所当然。`
    },
    {
      ...
      result:
        "网站从静态展示升级为可以长期扩展的内容地图，访问者能从不同路径理解我的能力。",
      featured: true,
      sortOrder: 4,
      content: `## 项目背景
...
- Keychain：保存 Bearer Token，避免敏感凭据明文保存。
...
这个项目训练的是端到端产品系统能力：不仅要写页面，还要处理认证、安全、数据一致性、实时事件、支付边界、双端 DTO 和部署运维。`
  ...
  const projectDepthAdditions: Record<string, string> = {
    "personal-intelligence-dashboard": `## 产品判断

我不希望它只是一个漂亮仪表盘。真正的个人工作台应该减少切换成本：今天要做什么、哪些想法值得继续、哪些内容需要复盘，都应该在一个地方形成闭环。

## 技术拆解

前台负责信息组织和输入效率，数据层负责项目、笔记、目标和复盘之间的关系，AI 只在需要展开、归纳和提醒时介入。这样它不会变成另一个噪音入口。

## 核心挑战

最大挑战是克制。个人效率工具很容易不断加功能，最后用户每天只是在维护系统。这个项目的判断标准是：每个模块是否让下一步行动更清楚。`,
    "brand-cms-creator-portfolios": `## 产品判断

创作者网站的核心不是一次性视觉，而是长期更新能力。前台要让访客愿意继续看，后台要让所有者愿意持续维护。

## 技术架构

我把文章、项目、动态、站点设置和复用视觉模块拆成清晰内容模型，用 Prisma 管理数据，用 Next.js App Router 组织前台和后台边界。

## 核心挑战

最大的难点是平衡表达力和可维护性。前台可以有流体渐变、动效和强视觉中心，但后台必须保持冷静、清晰、可预期。`,
    "automation-studio-repeated-work": `## 产品判断

自动化不是把脚本藏起来，而是让重复流程可见、可追踪、可恢复。只有团队知道任务现在处于什么状态，自动化才值得信任。

## 技术架构

系统围绕任务模板、执行记录、重试策略和提醒机制设计。每次运行都应该有输入、输出、状态和错误原因，而不是只留下成功或失败。

## 复盘

这个项目让我更重视可观测性。重复工作被自动化之后，责任并没有消失，它只是转移到了系统设计里。`,
    "bright-content-universe-website": `## 产品判断

yaokai.me 的重点不是更多页面，而是让访客能从不同入口理解同一个人：作品展示执行力，文章展示判断，指南展示方法，Now 展示当前状态。

## 技术架构

项目使用 Next.js、Prisma、Framer Motion 和 Tailwind，前台有明亮流体视觉系统，后台有 CMS 管理内容，部署脚本负责保留生产数据并完成迁移、构建、PM2 重启和 Nginx 反代。

## 核心挑战

最大挑战是不能只做表面升级。Dropdown 透明、鼠标黑点、重复文案、移动端拥挤和文章过短，都会让“高级感”失效。视觉、内容和工程验证必须一起完成。`,
    "machi-ios-native-city-life-app": `## 产品判断
...
离线优先的难点是恢复连接后的对齐。用户在本地做的动作、远端已经变化的状态、失败的同步任务，都需要明确策略。`,
    "machi-web-unified-python-backend": `## 产品判断
...
双端同步的关键是契约稳定。DTO 字段、分页规则、soft delete、权限判断和实时事件都要能被 Web 与 iOS 同时理解。`
  ...
  for (const project of projects) {
    const enhancedProject = {
      ...
      status: project.featured ? "Case Study" : "Private Project",
      screenshots: json([]),
      architectureNotes: projectDepthAdditions[project.slug]?.split("\n\n")[1]?.replace("## 技术架构\n\n", "").replace("## 技术拆解\n\n", "") || "",
      responsibilities: json([project.role]),
      keyChallenges: json([project.challenge]),
      solutions: json([project.solution]),
      technicalHighlights: json(project.techStack.slice(0, 5)),
      measurableResults: json([project.result]),
      nextSteps: json(["补充真实项目截图", "补充 Live Demo / GitHub 链接", "继续记录真实使用反馈和复盘"]),
      startDate: "",
      endDate: "",
      content: `${project.content}${projectDepthAdditions[project.slug] ? `\n\n${projectDepthAdditions[project.slug]}` : ""}`
    };
...
把这篇指南连接到具体项目或文章里，形成从方法到案例的探索路径。`;
  ...
  const nowItems = [
    ["重做明亮流体个人网站", "把个人网站从暗色作品集升级成可探索的内容地图。", "当前项目", "进行中"],
